...
export default function Home() {
  ...
  const remainingClients = clients.filter((client) => !featuredClientNames.includes(client.name) && client.asset);
  ...
  return <main>
    ...
    <section className="proof section"><div><div className="section-label">EMPRESAS</div><h2>Empresas que confiaron<br /><em>en nuestro equipo.</em></h2><p>Durante nuestra trayectoria trabajamos en eventos para cientos de empresas y organizaciones.</p></div><div><div className="featured-logo-grid" aria-label="Empresas que confiaron en nuestro equipo">{featuredClients.map((client) => <div className="featured-client-mark" key={client.name} title={client.name}><img src={client.asset} alt={client.name} loading="lazy" /></div>)}</div><div className="company-marquees" aria-label="Más empresas que confiaron en nuestro equipo"><div className="company-track">{[...remainingClients, ...remainingClients].map((client, index) => <div className="client-mark" key={`a-${index}`} title={client.name}><img src={client.asset} alt={client.name} loading="lazy" /></div>)}</div></div></div></section>
