...
export default function Home() {
  ...
  return <main>
    ...
    <section className="credentials"><div><b>17 años</b><span>de experiencia en eventos</span></div><div><b>+5.000</b><span>eventos realizados por nuestro equipo</span></div><div><b>+290</b><span>reseñas en Google</span></div><div><b>cientos</b><span>de empresas y organizaciones</span></div><p>Experiencia del equipo detrás de SYNC.</p></section>

    <section className="quick-trust"><div><p className="section-label">RESPALDO</p><h2>Una nueva propuesta del equipo de <em>Karaoken Shows.</em></h2></div><div><p>SYNC nace del equipo detrás de Karaoken Shows: 17 años produciendo entretenimiento para eventos, más de 5.000 eventos realizados y más de 290 reseñas en Google.</p><p className="trust-signoff">by Karaoken Shows</p><a className="text-link" href="#nosotros">Ver nuestra trayectoria <Arrow /></a></div></section>
    ...
    <section className="proof section"><div><div className="section-label">05 — EMPRESAS</div><h2>Empresas que confiaron<br /><em>en nuestro equipo.</em></h2><p>A lo largo de nuestra trayectoria trabajamos en eventos para empresas, instituciones y algunas de las marcas más reconocidas del país.</p></div><div className="company-marquees" aria-label="Empresas que confiaron en nuestro equipo"><div className="company-track">{[...clients, ...clients].map((client, index) => <div className="client-mark" key={`a-${index}`} title={client.name}>{client.asset ? <img src={client.asset} alt={client.name} loading="lazy" /> : <span>{client.name}</span>}</div>)}</div><div className="company-track reverse">{[...clients, ...clients].reverse().map((client, index) => <div className="client-mark" key={`b-${index}`} title={client.name}>{client.asset ? <img src={client.asset} alt={client.name} loading="lazy" /> : <span>{client.name}</span>}</div>)}</div></div></section>
    <section className="reviews section" data-review-widget-slot="google-reviews"><div><div className="section-label">06 — RESEÑAS</div><h2>Más de 290 reseñas<br /><em>en Google.</em></h2><p>Opiniones reales de clientes del equipo detrás de SYNC.</p><div className="google-proof"><img src="/clients/google.svg" alt="Google" /><span>★★★★★</span><strong>5.0</strong><small>+290 reseñas</small></div><a className="button review-cta" href="https://maps.app.goo.gl/djVvNZNHfwS8qsja9" target="_blank" rel="noreferrer">Ver todas las reseñas en Google <Arrow /></a></div>{reviews.length > 0 && <div className="review-slots">{reviews.map((review) => <article key={review.name}><span className="review-stars">★★★★★</span><p>{review.quote}</p><footer><b>{review.name}</b><small>{review.context}</small></footer></article>)}</div>}</section>
